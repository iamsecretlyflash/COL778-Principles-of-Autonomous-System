...
class MDPSolver :
    ...
    def heap_value_iteration(self, tol = 1e-9, max_state_updates = 1000, print_iters = False):
        ...
        value = np.zeros(self.num_states)
        value[list(self.environment.terminal)] = 0
        iter = 0
        pq = []
        history = [value[0]]
        t = time.time()
        while iter < max_state_updates:
            ...
            print(f'Intermediate Heap Step Done in : {time.time() - t2} seconds and {iter_local} state updates')
            if np.linalg.norm(val_val - value, ord = np.inf) < tol * (1-self.gamma) / self.gamma:
                break
        print(f'Total Heap Step Done in : {time.time() - t} seconds')
